...
class Monitor {
  ...
  configureSend() {
    this.send = ({ productUrl, productImage, description, title ,vidUrl}) => {
      sendDiscordWebhook({
...
let response = await twitterGetUrl(`https://twitter.com/${tweet.user.screen_name}/status/${tweet.id_str}`)
        console.log(response)
if (response.found && response.type != "image" ){
 mediaUrl= response.download[response.dimensionsAvailable -1].url;
      ...
      this.send({
        ...
        vidUrl: mediaUrl ?mediaUrl:undefined,
      });
